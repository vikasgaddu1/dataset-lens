...
async function compareImplementations(filePath) {
    console.log('\n' + '='.repeat(60));
    ...
    try {
        ...
        const pythonColNames = pythonMeta.variables.map((v) => v.name);
        const jsColNames = jsMeta.columns?.map((c) => c.name) || [];
        results.push({
        ...
        const pythonHasLabels = pythonMeta.variables.some((v) => v.label && v.label !== v.name);
        const jsHasLabels = jsMeta.columns?.some((c) => c.label && c.label !== c.name) || false;
        results.push({
        ...
        const pythonHasFormats = pythonMeta.variables.some((v) => v.format && v.format !== '');
        const jsHasFormats = jsMeta.columns?.some((c) => c.displayFormat || c.format) || false;
        results.push({
        ...
        results.forEach(r => {
            const match = r.match ? '✅' : '❌';
            console.log(r.feature.padEnd(maxFeatureLen) + ' | ' +
...
//# sourceMappingURL=compare-with-python.js.map