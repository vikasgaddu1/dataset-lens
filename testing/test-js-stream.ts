...
interface TestResults {
    ...
    sampleData: any[];
    uniqueValues: any;
    multiVariableUnique: any;
    performance: {
        metadataTime: number;
        dataReadTime: number;
        uniqueValueTime: number;
    };
}
        ...
        try {
            ...
            if (typeof this.reader[Symbol.asyncIterator] === 'function') {
                ...
                for await (const row of this.reader) {
                    ...
                    count++;
                    if (count >= 10) break; // Read first 10 rows
                }
        ...
        try {
            ...
            if (typeof this.reader.getUniqueValues === 'function') {
                ...
                if (meta?.variables && Array.isArray(meta.variables)) {
                    columnNames = meta.variables.map((v: any) => v.name || v);
...
.repeat(60));
    ...
    printSummary() {
        ...
        console.log('\n✅ Features Available:');
        if (this.results.metadata) console.log('  - Metadata extraction');
        if (this.results.sampleData.length > 0) console.log('  - Data reading');
        if (this.results.uniqueValues) console.log('  - Single variable unique values');
        if (this.results.multiVariableUnique) console.log('  - Multi-variable unique combinations');
        ...
        if (hasLabels && hasFormats && hasUnique) {
            ...
            if (!hasLabels) console.log('    - Variable labels');
            if (!hasFormats) console.log('    - Variable formats');
            if (!hasUnique) console.log('    - Unique value extraction');
